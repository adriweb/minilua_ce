...
static int countint(const TValue* key, int* nums)
{
    ...
    if (0 < k && k <= (1 << (32 - 2))) // todo: fixme (shift count >= width of type)
    {
        nums[ceillog2(k)]++;
        return 1;
    } else
...
//    luaS_fix(luaS_newliteral(L, "not enough memory"));
    tmp = (luaS_newlstr(L, "not enough memory", sizeof("not enough memory") - 1));
    tmp->tsv.marked |= (1 << 5);
...
static int str_format(lua_State* L)
{
    ...
    while (strfrmt < strfrmt_end)
    {
        ...
        else
        {
            ...
            {
                addintlen(form);
                sprintf(buff, form, (int) luaL_checknumber(L, arg)); // LLVM tmp workaround, cast to int
                //sprintf(buff, form, (long) luaL_checknumber(L, arg)); // LLVM FIXME
                break;
            }
            case 'o':
            case 'u':
            ...
            case 'X':
            {
                addintlen(form);
                sprintf(buff, form, (int) luaL_checknumber(L, arg)); // LLVM tmp workaround, cast to int
                //sprintf(buff, form, (unsigned long) luaL_checknumber(L, arg)); // LLVM FIXME
                break;
            }
            case 'e':
            case 'E':
...
*/
// FIXME ZDS string lib :(
static const luaL_Reg lualibs[] = {
    {"",       luaopen_base},
//    {"table",  luaopen_table}, // TODO: uncomment me
